                ...
                if($check_email){
                    ...
                    if($this->form_validation->run($sf, 'sf') == FALSE){
                        ...
                        if($batas >= $sekarang){
                            ...
                            if($kirim){
                                $model->tambah($sf);
                                session()->setFlashdata('success_sf', 'Terima kasih telah mendaftar Software Fair.  Silahkan cek inbox/spam pada email Anda untuk informasi lebih lanjut.');
                                return redirect()->to(base_url()."/#sf");
                            } else {
                ...
                if($check_email){
                    ...
                    if($this->form_validation->run($hf, 'hf') == FALSE){
                        ...
                        if($batas >= $sekarang){
                            ...
                            $email_smtp->setTo("$email");
                            $email_smtp->setSubject("Konfirmasi Pendaftaran Developer HI TECH 2021");
                            $email_smtp->setMessage("<div>Halo, Tim $tim</div><div><br /></div><div>Terimakasih telah mendaftar sebagai developer hardware fair di acara Hi-technology. Untuk para tim developer diharapkan untuk bergabung kedalam whatsapp group agar mendapatkan informasi-informasi terbaru.</div><div>Berikut link whatsapp group :</div><div><br />https://chat.whatsapp.com/CtJBWZtWbwB3hLVHAZyYx2</div><div><br /></div><div>Untuk panduan lebih lengkap, Developer bisa membaca guideline melalui link berikut:</div><div>".base_url("assets/guideline/GUIDELINE_DEVELOPER_SF-HF_2021.pdf")."</div><br /><div>Salam, Hi-technology 2021</div>");
                            $kirim = $email_smtp->send();
                            if($kirim){
                                $model->tambah($hf);
